function output_map = ukf_balance(input_data, input_start, input_finish, input_mean, input_judge_stop_times)
...
%%%
        ...
        % Status 状态 判断
            ...
            input_original_v_Xukf = original_v_Xukf(2,:);
            input_velocity = Xukf(2,:);
            input_distance = Zd(1:N);
            input_all_size = N;
            output_status = status_judgement(input_all_size, input_tail_size, input_accelerate, input_original_v_Xukf, input_velocity, input_distance);
        %

        % Map 赋值
            % 状态
            output_map('output_status') = output_status;
            % 末端d
            index_compensated_v_last_zero = (zero_judgement(Xukf(2,:)));
            i_c_v_l_z = index_compensated_v_last_zero(end);
            output_map('output_tail_d') = mean( Zd( i_c_v_l_z-5:min(i_c_v_l_z+5,end) , 1) );
        %
    %
end
