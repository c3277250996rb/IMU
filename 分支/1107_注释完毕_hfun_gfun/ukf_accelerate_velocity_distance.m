...
Xsigma_0    = zeros(L,3);
Xaugsigma_0 = zeros(L,3);
Zsigmapre_0 = zeros(L,3);
T = 0.01;
...
% 2 将Sigma点通过状态转移函数 gfun(x,t) 预测得到:
    ...
    %                   v0      , a0      , accelerate 
    Xsigmapre = my_gfun(Xsigma, accelerate(t - 1), accelerate(t), T);
...
% 5 将Sigma点代入观测函数 hfun(x,t) 计算预测观测值
    ...
    % Zsigmapre(1,k) 将计算得到的观测值存储在 Zsigmapre 的第一行和第 k 列中。这样，Zsigmapre 就会存储所有 sigma 点对应的观测预测值。
            ...
            %                        a0              , v0              , velocity
            Zsigmapre(1,k) = my_hfun(Zsigmapre_0(1,k), Xaugsigma_0(:,k), Xaugsigma(:,k), T);
        end
...
plot(t,velocity(1,:),'-or','LineWidth',1);%观测轨迹
% plot(t,v_,'-or','LineWidth',1);%观测轨迹
