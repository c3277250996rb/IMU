...
float DM_angle_record[10][5] = {0};  // 给 data management 用的二维数组
...
void imu_convert_9axes(void){
    ...
    #ifdef use_DM_Write
        for(i = 0; i < 3; i++){
            euler_angle[i] = Q_current_index;
        }
        if(is_Q_stop == enum_Ready){
            is_Q_stop = eunm_Continuous;
        }else if((is_Q_stop == eunm_Continuous) && (Q_current_index < 10)){
            // DM_angle_record[Q_current_index][0] = FATFS_Index_Current * 100000 + Q_current_index;
            // DM_angle_record[Q_current_index][0] = (float) FATFS_Index_Current;
