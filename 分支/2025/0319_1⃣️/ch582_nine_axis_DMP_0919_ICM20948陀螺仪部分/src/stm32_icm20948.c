...
extern uint32_t get_system_timer_tick_ms(void);
...
int idd_io_hal_read_reg(void *context, uint8_t reg, uint8_t *rbuffer, uint32_t rlen)
{
    ...
    i2c_read_bytes(ICM_I2C_ADDR_REVA << 1, reg, rbuffer, rlen);
   // HAL_I2C_Mem_Read(&ICM20948_I2C, ICM_I2C_ADDR_REVA << 1, reg, I2C_MEMADD_SIZE_8BIT, rbuffer, rlen, HAL_MAX_DELAY);

    return 0;
}

int idd_io_hal_write_reg(void *context, uint8_t reg,  uint8_t *wbuffer, uint32_t wlen)
{
    (void)context;

    i2c_write_bytes(ICM_I2C_ADDR_REVA << 1, reg, wbuffer, wlen);
...
void check_rc(int rc, const char *msg_context)
{
    if (rc < 0) {
        ...
        UART1_SendString((uint8_t *)"ICM20948 ERROR!", 16);
        while (1) ;
    }
}
...
{
    return get_system_timer_tick_ms();
}
...
int icm20948_sensor_setup(void)
{
   ...
   // HAL_UART_Transmit(&ICM20948_UART, UARTbuf, 29, 100);
    ...
    printf("Putting Icm20948 in sleep mode...\r\n");
