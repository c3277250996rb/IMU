...
// 在这里执行 read 命令的逻辑
    ...
    if (argc >= 1)
    {
    	...
    	if(strcmp(*argv,"mag")==0)//cmd cali mag
    	{
...
// 在这里执行 read 命令的逻辑
    ...
    if (argc >= 1)
    {
    	...
    	if(strcmp(*argv,"acc")==0)//cmd ouput acc
    	{

    		imu_9.shell_cmd_ok = 1;
    		imu_9.output_flag = 1;
    		imu_9.output_mode = 3;
    		shell_printf("output acc ,mode %d \r\n",imu_9.output_mode);
    	}
    	if(strcmp(*argv,"iacc")==0)//cmd ouput acc
        {
