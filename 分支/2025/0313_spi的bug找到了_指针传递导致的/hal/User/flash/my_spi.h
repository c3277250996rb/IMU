...
#define SPI_FLASH_CS_LOW()      SPI_GPIO_Write_CS_0(SPI_Class_1)
#define SPI_FLASH_CS_HIGH()     SPI_GPIO_Write_CS_1(SPI_Class_1)
...
#define debug10
    ...
    // SPI_FLASH_BufferWrite(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite);
    void SPI_FLASH_BufferWrite(uint8_t* input_Pointer, uint32_t WriteAddress, uint16_t WriteSize);
#else
...
#define __SPI__Register__Definition__
    ...
    uint8_t SPI_Register_Write_Read(SPI_Class_Struct input_SPI_Class, uint8_t input_Data);
     ...
     uint8_t SPI_Register_Timeout(SPI_Class_Struct input_SPI_Class, uint8_t input_Message);
