...
char readbuffer[512];    
char default_write_buffer[] = "100001";
...
void FATFS_Write(char* path, char* write_buffer){
    f_close(&fnew);
...
void FATFS_Muti_Test(void){
    ...
    FATFS_Read(my_path);
    FATFS_Write(my_path, default_write_buffer);
...
FATFS_Result_Struct FATFS_Index_Check(void){
    ...
    if(is_write_next_index){
        // int_to_string_with_snprintf(strtol(temp_buffer, NULL, 10), temp_to_write);
        sprintf(temp_to_write, "\n%s", temp_buffer);
        FATFS_Write(index_check_path, temp_to_write);
    }else{
        FATFS_Write(index_check_path, default_write_buffer);
    }
