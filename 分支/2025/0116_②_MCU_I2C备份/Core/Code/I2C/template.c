#include "main.h"


void I2C_APP_Write(input_I2C_Class, DeviceAddress, RegisterAddress, input_Data){
    I2C_Driver_Start();
	I2C_Driver_WriteByte(DeviceAddress | I2C_Direction_Transmitter);
	I2C_Driver_WaiteForAck();
	I2C_Driver_WriteByte(RegisterAddress);
	I2C_Driver_WaiteForAck();
	I2C_Driver_WriteByte(input_Data);
	I2C_Driver_WaiteForAck();
	I2C_Driver_Stop();
}


void I2C_Driver_Start();
void I2C_Driver_Ack();
void I2C_Driver_Write(input_I2C_Class, input_Data){
	uint8_t i;
    GPIO_TypeDef temp_SDA_Port  = input_I2C_Class->SDA_Port;
    uint16_t temp_SDA_Pin       = input_I2C_Class->SDA_Pin;
	
	I2C_GPIO_SDA_Mode_OUT();
	for(i = 0; i < 8; i ++){
		I2C_GPIO_SCL_Write_0();
		Delay_us(1);

		if(input_Data & 0x80){
			I2C_GPIO_SDA_Write_1();
        }else{
			I2C_GPIO_SDA_Write_0();
        }

		input_Data <<= 1;
		Delay_us(1);
		I2C_GPIO_SCL_Write_1();
		Delay_us(1);
	}

}





#define I2C_GPIO_SCL_Write_1 I2C_GPIO_SCL_Write(1)
#define I2C_GPIO_SCL_Write_0 I2C_GPIO_SCL_Write(0)
#define I2C_GPIO_SDA_Write_1 I2C_GPIO_SDA_Write(1)
#define I2C_GPIO_SDA_Write_0 I2C_GPIO_SDA_Write(0)

#define I2C_GPIO_SDA_Mode_IN  I2C_GPIO_SDA_Mode(enum_INPUT)
#define I2C_GPIO_SDA_Mode_OUT I2C_GPIO_SDA_Mode(enum_OUTPUT)



void I2C_GPIO_SCL_Write();
void I2C_GPIO_SDA_Write();
void I2C_GPIO_SDA_Mode(input_I2C_Class, input_SDA_Mode);
GPIO_PinState I2C_GPIO_SDA_Read(input_I2C_Class){
    GPIO_PinState output_PinState;
    GPIO_TypeDef temp_SDA_Port  = input_I2C_Class->SDA_Port;
    uint16_t temp_SDA_Pin       = input_I2C_Class->SDA_Pin;

    output_PinState = HAL_GPIO_ReadPin(temp_SDA_Port, temp_SDA_Pin);
    return output_PinState;
}















