...
FATFS_Result_Struct FATFS_Read(char* path){
    FATFS_Result_Struct temp_FATFS_Result = FATFS_Result_default;
    f_close(&fnew);
...
FATFS_Result_Struct FATFS_Index_Check(void){
    ...
    if(temp_FATFS_Result.File_Size > 0){
        ...
        printf("[%s]%d: %d", __FILE__, __LINE__, is_write_next_index);
    }
    ...
    if(is_write_next_index){
        ...
        printf("\r\n%d\r\n", temp_num);
        // FATFS_Index_Current = ((uint8_t) strtol(temp_buffer, NULL, 10))+1;
        FATFS_Index_Current = temp_num + 1;
        printf("[%s]%d: %d", __FILE__, __LINE__, FATFS_Index_Current);
        sprintf(temp_to_write, "%d", FATFS_Index_Current);
        printf("[%s]%d: %s", __FILE__, __LINE__, temp_to_write);
        FATFS_Write(index_check_path, "\r\n", -1);
