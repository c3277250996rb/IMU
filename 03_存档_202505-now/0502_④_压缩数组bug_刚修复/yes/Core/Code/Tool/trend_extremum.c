...
#define debug_TE_compress

#ifdef debug_TE_compress
...
#ifdef debug_TE_compress
    printf("arr: [");
    for (int j = 0; j < Max_Q_Row; j++) {
        printf("%.2f", arr[j]);
        if (j != Max_Q_Row - 1) printf(", "); // 如果不是最后一个，加逗号
    }
    printf("]\r\n");
...
void TE_ReInit(void){
    ...
    // memcpy(trend, 0, Max_Q_Row);
    // memcpy(extremum, 0, Max_Q_Row);
    for (size_t i = 0; i < Max_Q_Row; i++)
    {
        trend[i] = 0;
        extremum[i] = 0.0F;
    }
