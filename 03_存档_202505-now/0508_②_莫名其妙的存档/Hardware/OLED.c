...
#include <stdio.h>
#include <stdarg.h>
uint8_t my_string_len = 0;
uint8_t OLED_DisplayBuf[8][128];

void oled_i2c_writecommand(I2C_DEPLOY* bus, uint8_t Command) {
	bus->WriteReg(bus, 0x00, Command);
}

void oled_i2c_writedata(I2C_DEPLOY* bus, uint8_t *Data, uint8_t Count) {
	for (uint8_t i = 0; i < Count; i++) {
		bus->WriteReg(bus, 0x40, Data[i]);
	}
}

void OLED_Init(I2C_DEPLOY* bus)
{
	Delay_ms(50);//等待oled上电
	//写命令，对OLED进行初始化配置
	oled_i2c_writecommand(bus, 0xAE);	//设置显示开启/关闭，0xAE关闭，0xAF开启
	oled_i2c_writecommand(bus, 0xD5);	//设置显示时钟分频比/振荡器频率
	oled_i2c_writecommand(bus, 0x80);	//0x00~0xFF
	oled_i2c_writecommand(bus, 0xA8);	//设置多路复用率
	oled_i2c_writecommand(bus, 0x3F);	//0x0E~0x3F
	oled_i2c_writecommand(bus, 0xD3);	//设置显示偏移
	oled_i2c_writecommand(bus, 0x00);	//0x00~0x7F
	oled_i2c_writecommand(bus, 0x40);	//设置显示开始行，0x40~0x7F
	oled_i2c_writecommand(bus, 0xA1);	//设置左右方向，0xA1正常，0xA0左右反置
	oled_i2c_writecommand(bus, 0xC8);	//设置上下方向，0xC8正常，0xC0上下反置
	oled_i2c_writecommand(bus, 0xDA);	//设置COM引脚硬件配置
	oled_i2c_writecommand(bus, 0x12);
	oled_i2c_writecommand(bus, 0x81);	//设置对比度
	oled_i2c_writecommand(bus, 0xCF);	//0x00~0xFF
	oled_i2c_writecommand(bus, 0xD9);	//设置预充电周期
	oled_i2c_writecommand(bus, 0xF1);
	oled_i2c_writecommand(bus, 0xDB);	//设置VCOMH取消选择级别
	oled_i2c_writecommand(bus, 0x30);
	oled_i2c_writecommand(bus, 0xA4);	//设置整个显示打开/关闭
	oled_i2c_writecommand(bus, 0xA6);	//设置正常/反色显示，0xA6正常，0xA7反色
	oled_i2c_writecommand(bus, 0x8D);	//设置充电泵
	oled_i2c_writecommand(bus, 0x14);
	oled_i2c_writecommand(bus, 0xAF);	//开启显示
	
	OLED_Clear();				//清空显存数组
	OLED_Update(bus);				//更新显示，清屏，防止初始化后未显示内容时花屏
}
...
/**
  ...
  */
void OLED_SetCursor(I2C_DEPLOY* bus, uint8_t Page, uint8_t X)
{
...
//	X += 2;
	...
	/*通过指令设置页地址和列地址*/
	oled_i2c_writecommand(bus, 0xB0 | Page);					//设置页位置
	oled_i2c_writecommand(bus, 0x10 | ((X & 0xF0) >> 4));	//设置X位置高4位
	oled_i2c_writecommand(bus, 0x00 | (X & 0x0F));			//设置X位置低4位
}
...
/**
  ...
  */
void OLED_Update(I2C_DEPLOY* bus)
{
	uint8_t j;
	/*遍历每一页*/
	for (j = 0; j < 8; j ++)
	{
		/*设置光标位置为每一页的第一列*/
		OLED_SetCursor(bus, j, 0);
		/*连续写入128个数据，将显存数组的数据写入到OLED硬件*/
		oled_i2c_writedata(bus, OLED_DisplayBuf[j], 128);
	}
}
...
/**
  ...
  *           故调用显示函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_UpdateArea(I2C_DEPLOY* bus, int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	int16_t j;
	int16_t Page, Page1;
	...
	for (j = Page; j < Page1; j ++)
	{
		...
		{
			/*设置光标位置为相关页的指定列*/
			OLED_SetCursor(bus, j, X);
			/*连续写入Width个数据，将显存数组的数据写入到OLED硬件*/
			oled_i2c_writedata(bus, &OLED_DisplayBuf[j][X], Width);
		}
	}
}
...
void my_oled_shownum_debug(int16_t X, int16_t Y, char *String, uint32_t Number, uint8_t Length)
{
	OLED_ShowString(X, Y, String, OLED_8X16);
	OLED_ShowNum(X + my_string_len, Y, Number, Length, OLED_8X16);
}

void my_oled_showslo_debug(int16_t X, int16_t Y, char *String, float Number, uint8_t Length)
{
	OLED_ShowString(X, Y, String, OLED_8X16);
	OLED_ShowFloatNum(X + my_string_len, Y, Number, 3, Length, OLED_8X16);
}
