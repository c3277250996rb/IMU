...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
    ...
    if (huart->Instance == USART1) {  // 如果是串口1
        ...
        if ((Buffer_Receive_Register & 0x8000) == 0) {  // 接收未完成

            if (Buffer_Receive_Register & 0x4000) {  // 接收到了0x0d
			
                if (Buffer_Receive_IT[0] != 0x0a) {
                    Buffer_Receive_Register = 0;  // 接收错误, 重新开始
                } else {
                    Buffer_Receive_Register |= 0x8000;  // 接收完成了
