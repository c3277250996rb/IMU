/**
  ******************************************************************************
  * @file    I2C.h
  * @author  Waveshare Team
  * @version V1.0
  * @date    29-August-2014
  * @brief   This file contains all the functions prototypes for the I2C firmware 
  *          library.

  ******************************************************************************
  * @attention
  *
  * THE PRESENT FIRMWARE WHICH IS FOR GUIDANCE ONLY AIMS AT PROVIDING CUSTOMERS
  * WITH CODING INFORMATION REGARDING THEIR PRODUCTS IN ORDER FOR THEM TO SAVE
  * TIME. AS A RESULT, WAVESHARE SHALL NOT BE HELD LIABLE FOR ANY
  * DIRECT, INDIRECT OR CONSEQUENTIAL DAMAGES WITH RESPECT TO ANY CLAIMS ARISING
  * FROM THE CONTENT OF SUCH FIRMWARE AND/OR THE USE MADE BY CUSTOMERS OF THE
  * CODING INFORMATION CONTAINED HEREIN IN CONNECTION WITH THEIR PRODUCTS.
  *
  ******************************************************************************
  */


#ifndef _I2C_H_
#define _I2C_H_

#include "Public_StdTypes.h"
#include "my_i2c.h"

#define I2C_SCL_PIN  GPIO_Pin_10
#define I2C_SDA_PIN  GPIO_Pin_11

#if old

// #define I2C_SCL_Set()  GPIO_WriteBit(GPIOB, I2C_SCL_PIN, Bit_SET)
#define I2C_SCL_Set()  I2C_GPIO_SCL_Write_1(I2C_Class_2)
// #define I2C_SCL_Set()  I2C_GPIO_SCL_Write(I2C_Class_2, 1)
#define I2C_SCL_Clr()  GPIO_WriteBit(GPIOB, I2C_SCL_PIN, Bit_RESET)

#define I2C_SDA_Set()  GPIO_WriteBit(GPIOB, I2C_SDA_PIN, Bit_SET)
#define I2C_SDA_Clr()  GPIO_WriteBit(GPIOB, I2C_SDA_PIN, Bit_RESET)

#define I2C_SDA_Get()  GPIO_ReadInputDataBit(GPIOB, I2C_SDA_PIN)

#else

#define I2C_SCL_Set()  I2C_GPIO_SCL_Write_1(I2C_Class_2)
#define I2C_SCL_Clr()  I2C_GPIO_SCL_Write_0(I2C_Class_2)

#define I2C_SDA_Set()  I2C_GPIO_SDA_Write_1(I2C_Class_2)
#define I2C_SDA_Clr()  I2C_GPIO_SDA_Write_0(I2C_Class_2)

#define I2C_SDA_Get()  I2C_GPIO_SDA_Read(I2C_Class_2)
// #define I2C_SDA_Get()  GPIO_ReadInputDataBit(GPIOB, I2C_SDA_PIN)

#endif



enum
{
	I2C_SDA_IN,
	I2C_SDA_OUT
};

enum
{
	I2C_ACK,
	I2C_NACK
};


#if old
extern void I2C_SDAMode(uint8_t Mode);
extern void I2C_Stop(void);
extern void I2C_Start(void);

extern bool I2C_WaiteForAck(void);
extern void I2C_Ack(void);
extern void I2C_NAck(void);
#else

#define I2C_Stop() I2C_Driver_Stop(I2C_Class_2)
#define I2C_Start() I2C_Driver_Start(I2C_Class_2)
#define I2C_SDAMode(Mode) I2C_GPIO_SDA_Mode(I2C_Class_2, Mode)

#define I2C_WaiteForAck() I2C_Driver_WaitForAck(I2C_Class_2)
#define I2C_Ack() I2C_Driver_Ack(I2C_Class_2)
#define I2C_NAck() I2C_Driver_NAck(I2C_Class_2)
#endif

// extern bool I2C_WriteOneBit(uint8_t DevAddr, uint8_t RegAddr, uint8_t BitNum, uint8_t Data);
// extern bool I2C_WriteBits(uint8_t DevAddr, uint8_t RegAddr, uint8_t BitStart, uint8_t Length, uint8_t Data);


#if old

extern void I2C_WriteByte(uint8_t Data);
extern uint8_t I2C_ReadByte(uint8_t Ack);
extern void I2C_WriteOneByte(uint8_t DevAddr, uint8_t RegAddr, uint8_t Data);
extern uint8_t I2C_ReadOneByte(uint8_t DevAddr, uint8_t RegAddr);

#else

#define I2C_WriteByte( Data) I2C_Driver_Write(I2C_Class_2, Data)
#define I2C_ReadByte( Ack) I2C_Driver_Read(I2C_Class_2, Ack)
#define I2C_WriteOneByte( DevAddr,  RegAddr,  Data) I2C_APP_Write_Single_Byte(I2C_Class_2, DevAddr,  RegAddr,  Data)
#define I2C_ReadOneByte( DevAddr,  RegAddr) I2C_APP_Read_Single_Byte(I2C_Class_2, DevAddr,  RegAddr)

#endif



// extern bool I2C_WriteBuff(uint8_t DevAddr, uint8_t RegAddr, uint8_t Num, uint8_t *pBuff);
// extern bool I2C_ReadBuff(uint8_t DevAddr, uint8_t RegAddr, uint8_t Num, uint8_t *pBuff);

extern void I2Cx_Init(void);
extern void Delay_us(uint16_t Time);
extern void Delay_ms(uint16_t Time);

#endif

/******************* (C) COPYRIGHT 2014 Waveshare *****END OF FILE*******************/

