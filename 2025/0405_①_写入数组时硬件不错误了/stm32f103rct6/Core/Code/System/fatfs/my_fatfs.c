 ...
 * @retval None
 */
void FATFS_Write(char* path, char* write_buffer, int write_position, uint8_t is_echo){
    // 1.关闭文件 (保险)
    f_close(&f_default);
...
void FATFS_Muti_Test(void){
    ...
    // 5.写入文件并回显
    FATFS_Write(my_path, Index_Text_Default, -1, 1);
...
void FATFS_Index_Check(uint8_t is_append){
    ...
    if(is_append){
        if(is_write_next_index){
            ...
            FATFS_Write(index_check_path, temp_to_write, -1, 0);
        }else{
            /* 不需要追加索引, 直接写入默认索引10001 */
            FATFS_Index_Current = 10001;
            FATFS_Write(index_check_path, Index_Text_Default, -1, 1);
        }
