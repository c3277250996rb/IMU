...
void write_imu_array_to_file(void){
    ...
    // [b.1] 如果是静止
    if(enum_RESET == is_WF_trigger){
        /* 判断开始: 静止 -> 运动 */
        WF_Judge_From_Static_To_Dynamic(buffer_9axes[0]);
    }

    // [b.2] 如果是运动
    if(enum_TRIGGER == is_WF_trigger || enum_ADDUP == is_WF_trigger){
        /* 判断结束: 运动 -> 静止 */
        WF_Judge_From_Dynamic_To_Static(buffer_9axes[0]);

    // [b.3] 如果运动结束了
        if(enum_FINISH == is_WF_trigger){
            is_Q_stop = enum_Stop;
            is_WF_trigger = enum_RESET;
        }

    // [b.4] 如果正在运动 或者 被强制停止了
        if(eunm_Continuous == is_Q_stop || enum_Stop == is_Q_stop){
        // [a.1] 如果赋值赋满了就写进去 (或者被强制停止了)
            if(Cache_Max_Row <= Q_current_index || enum_Stop == is_Q_stop){
        ...
        // [a.4] 打印信息
        /* info */ char *temp_string = muti_printf_arrays(1, ",", "%f",
        /* info */     euler_angle, 3
        /* info */ );
        /* info */ printf("采集: %3d %s\r\n", Q_current_index, temp_string);
    }
}
