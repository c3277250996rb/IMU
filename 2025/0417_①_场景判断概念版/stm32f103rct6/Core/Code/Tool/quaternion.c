...
#define use_DM_Write
// float DM_angle_record[80][Info_Column] = {0};  // 给 data management 用的二维数组
float DM_angle_record[160][Info_Column] = {0};  // 给 data management 用的二维数组
...
void imu_convert_9axes(void){
    ...
    for(i = 0; i < 3; i++){
        ...
        for(i = 0; i < 3; i++){
            ...
            euler_angle[i] = buffer_9axes[i];
        }

        #include "windows_filter.h"
        if(is_WF_trigger == enum_RESET){
            WF_Judge_From_Static_To_Dynamic(euler_angle[0]);
        }

        if(is_WF_trigger == enum_TRIGGER || is_WF_trigger == enum_ADDUP){
            // 1.判断: 运动 -> 静止
            WF_Judge_From_Dynamic_To_Static(euler_angle[0]);

            // 2.判断是否结束
            if(is_WF_trigger == enum_FINISH){
                is_Q_stop = enum_Stop;
                is_WF_trigger = enum_RESET;
            }
            ...
            if(is_Q_stop == enum_Ready){
                ...
                Q_current_index = (Q_current_index + 1) % Cache_Max_Row;
            }else{
            ...
            // printf("采集: %3d,%6.1f,%6.1f,%6.1f\r\n", Q_current_index, euler_angle[0], euler_angle[1], euler_angle[2]);
            printf("采集: %3d ,%f,%f,%f\r\n", Q_current_index, euler_angle[0], euler_angle[1], euler_angle[2]);
        }
